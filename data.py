...
def get_transforms(model_type):
    if model_type == "clip":
        ...
        train_transform = tv.transforms.Compose([
            tv.transforms.Lambda(convert_rgb),
...
def raw_image_collate_fn(batch):
    ...
    if len(batch[0]) == 3:
        is_reals = torch.tensor([item[2] for item in batch])
        return images, labels, is_reals
...
def get_data_loader(
    ...
    collate_fn = raw_image_collate_fn if model_type == "qwen" else None
    ...
    if is_synth_train:
        ...
        if dataset == 'imagenet':
            ...
            # This must be downloaded by hand and filtered because links to the Cars dataset are broken and it is not available in torchvision.datasets.
